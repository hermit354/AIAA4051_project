...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/outputs/phase2_prompting_finetuned}"
...
checkpoint_for_model() {
  case "$1" in
    t5-small|t5-base|bart-base)
      printf "%s/outputs/phase1_baselines_converged/%s/best" "${PROJECT_ROOT}" "$1"
      ;;
    flan-t5-small|flan-t5-base)
      printf "%s/outputs/phase1_baselines_flan_bf16/%s/best" "${PROJECT_ROOT}" "$1"
      ;;
    *)
...
for model in "${MODELS[@]}"; do
  ...
  if [ ! -d "${checkpoint}" ]; then
    echo "missing checkpoint for ${model}: ${checkpoint}" >&2
    exit 1
  fi
